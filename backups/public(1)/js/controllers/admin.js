myApp.controller('AdminController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseArray', '$firebaseObject', 'Authentication'
    ...
    $scope.addProduct = function (item) {
        ...
        $scope.addProdform.prodname = null;
        $scope.addProdform.prodprice = null;
        ...
        uploadTask.on('state_changed', function progress(snapshot) {
            ...
            $('.close').on('click', function () {
                ...
                $('#pf_modalYes').on('click', function () {
                    // Delete the file
                    storageRef.delete().then(function () {
                        document.getElementById("addProductImage").src = '/images/dominic_profile.png';
                        $('#produploader').val(0);
                        $("#fileProdButton").val('');
                    }).catch(function (error) {
                        /* console.log('delete pic NOT successful'); */
