myApp.controller('ProfileController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseObject', 'Authentication', '$firebaseArray',
  ...
  function ($scope, $rootScope, $firebaseAuth, $firebaseObject, Authentication, $firebaseArray) {
    ...
    function loadSchedule(inDate) {
        ...
        // Cancelled appointments filter -- only show cancels for this specialist
        $scope.filterOnSpecialist = function(item) {
           
            console.log( item.specialist.firstname + ' ' + item.specialist.lastname);
            console.log($scope.user);
            return ((item.specialist.firstname === $scope.user.firstname) && (item.specialist.lastname === $scope.user.lastname));
        }
